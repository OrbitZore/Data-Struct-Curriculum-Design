...
struct segtree{
    ...
    int s,fsum,t;
    static int getfsum(unique_ptr<segtree> &ptr){
    	if (ptr) return ptr->fsum;
    	return 0;
    }
    template<class U>
    segtree(const U& op,const U& ed){
   		ll cnt=ed-op;
    	ll mid=cnt>>1;
   		s=op[mid];t=1;
...
struct solution{
	...
	void scan(){
		if (!(cin >> n >> m >> x >> a >> b >> M)) exit(0);	
		vec<int> s(n);iota(all(s),1);tree.reset(new segtree(all(s)));
	}
