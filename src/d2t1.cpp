...
struct solution{
	...
	int cmp(const string &A,const string &B){
		...
		vector<array<int,2>> DP(b.size()+1,{0,0});
		array<int,2>* const dp=&DP[1];
		for (int i=0;i<b.size();i++){
			if (b[i]==a[i]){
				cmax(dp[i][0],dp[i-1][0]+1);
				cmax(dp[i][1],dp[i-1][0]);
			}
			if (b[i]==a[i+1]){
				cmax(dp[i][0],dp[i-1][0]);
				cmax(dp[i][1],max(dp[i-1][1],dp[i-1][0])+1);
			}
		}
		return DP[b.size()][0]==b.size()||DP[b.size()][1]==b.size();
	}
	void solve(){
		string a;
		while (getline(cin,a),a!="#"){
			...
			for (int i=0;i<v.size();i++)
				if (a==v[i]){
					...
					if (cmp(a,v[i])||cmp(v[i],a)) vs.pb(v[i]);
				}
			print("{}:{ {}}\n",a,vs);
end:		;
		}
